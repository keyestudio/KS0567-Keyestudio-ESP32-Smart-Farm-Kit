...
### Étape 2 Installer le cadre de fixation pour le boîtier de batterie et installer la cabine d'alimentation, connecter la carte ESP32 et le module relais
...
>
> 1. **Ne réglez PAS le servo à 180°.** Le mécanisme de la porte ne nécessite pas la plage complète de 0° à 180°. Le régler à l'extrémité extrême fera que le servo appuiera contre la butée mécanique, ce qui pourrait surchauffer et endommager de façon permanente le moteur du servo.
> 2. **Gardez la carte alimentée pendant l'installation.** Lorsque le servo est alimenté et maintient son angle, ne forcez PAS l'engrenage ou la porte à la main. Si la carte n'est pas alimentée, le servo n'a pas de couple de maintien, et la rotation manuelle de l'engrenage déplacera l'angle calibré, provoquant un désalignement après la mise sous tension.
> 3. **N'utilisez PAS la force brute.** Si la porte ne bouge pas en douceur, arrêtez immédiatement et vérifiez l'assemblage. Forcer la porte endommagera les engrenages du servo.
...
**Connectez le servo à la carte de développement ESP32 sur la broche IO26.**
...
- Ouvrez **KidsBlock**. Sélectionnez la carte ESP32 et le bon port série, et téléchargez le code suivant sur la carte. Le code règle le servo à **165°**.

![image-20250425134842857](../media/image-20250425134842857.png)

![img](../media/image028-2.png)

> **Important :** Gardez la carte alimentée et le servo maintenu à 165° pendant que vous passez à l'étape d'assemblage. Ne déconnectez **pas** l'alimentation et ne déplacez **pas** manuellement l'engrenage tant que le panneau de porte n'est pas entièrement installé.
...
**Remarque : Les vis doivent être serrées pour maintenir le servo stable, sinon la porte pourrait se bloquer.**
...
Ne tournez pas l'engrenage après l'avoir installé sur le servo. Si vous avez déjà tourné l'engrenage, vous devrez réajuster l'angle du servo à 180°.
...
#### 4.13 Test de la porte


1. Connectez le servo à la broche IO26 de la carte ESP32. Connectez le jaune à S, le rouge à V, le noir à G.

![img](../media/image028-1.png)

2. Connectez 6 piles AA au port DC 7-12V de la carte ESP32. (Piles non incluses dans le kit)

![image-20250417084747308](../media/image-20250417084747308.png)

3. Téléchargez le code de test

A. Connectez la carte ESP32 à l'ordinateur avec le câble USB. Ouvrez le fichier INO dans le dossier **6.1Servo.sb3** avec KidsBlock.

![image-20250427084645025](../media/image-20250427084645025.png)

B. Téléchargez le code

![image-20250417085407205](../media/image-20250417085407205.png)

> **⚠ NOTE :** Après avoir téléchargé le code, si la porte ne peut pas être ouverte et fermée en douceur et que le servo est chaud, veuillez **couper l'alimentation immédiatement** pour éviter la surchauffe du servo. Vérifiez ensuite les points ci-dessous.
>
> **N'utilisez PAS de force brute** pour déplacer la porte ou l'engrenage à tout moment.

**Vérifier :**

1. Si la porte en plastique a un bon contact et des points de force avec la structure d'engrenage du servo.
1. Si la pointe de la vis de fixation sur la structure d'engrenage du servo est bloquée avec la coque en plastique du servo. Si c'est le cas, veuillez desserrer un peu la vis de fixation pour éviter que sa pointe ne touche le servo.
...
**Pour l'écran LCD, connectez le vert à SCL, le bleu à SDA, le rouge à VCC, le noir à GND.**
...
**Faites attention à la couleur du fil Dupont : Pour le module ultrasonique, connectez le bleu à TRIG, le vert à ECHO, le rouge à VCC, le noir à GND.**
...
**Faites attention à la couleur du fil Dupont : Connectez le jaune à S, le rouge à V, le noir à G.**
...
| Module | Fil |
| --- | --- |
| Ventilateur | 4 broches **Divisé (Noir-Rouge-Bleu-Vert)** |
| Capteur de vapeur | 3 broches 15cm |
| Photorésistance | 3 broches 15cm |
...
Passez le fil Dupont connecté au ventilateur à travers le trou **marqué 30** sur la planche en bois.
...
Faites passer le fil Dupont connecté au capteur de mouvement PIR à travers le trou marqué 24 sur la planche en bois.

| Composant | Fil | Broche de la carte ESP32 |
| --- | --- | --- |
| Capteur de mouvement PIR | 3PIN 15cm | io23 |
...
Faites passer le fil Dupont connecté au module bouton à travers le trou marqué 25 sur la planche en bois.

| Composant | Fil | Broche de la carte ESP32 |
| --- | --- | --- |
| Bouton | 3PIN 15cm | io5 |
...
| --- | --- | --- |
| Module Ultrasonique | 4PIN **Divisé** ( Noir-Vert-Bleu-Rouge) | io13(ECHO), io12(TRIG) |
...
| Composant | Fil | Broches de la carte ESP32 |
| --- | --- | --- |
| LCD1602 | 4PIN **Connecté** **(Noir-Rouge-Bleu-Vert)** | I2C |
...
Faites passer le fil Dupont connecté au module bouton à travers le trou marqué 20 sur la planche en bois.

| Composant | Fil | Broches de la carte ESP32 |
| --- | --- | --- |
| Capteur de température et d'humidité | 3PIN 20cm | io17 |
...
| Composant | Fil | Broche de la carte ESP32 |
| --- | --- | --- |
| Capteur de vapeur | 3PIN 15cm | io35 |
...
| Composant | Fil | Broche de la carte ESP32 |
| --- | --- | --- |
| Photorésistance | 3PIN 15cm | io34 |
...
**Faites passer le fil du Servo par le Trou 15, puis connectez-le à la carte ESP32.**
...
| Broche de la carte | Couleur du fil |
| --- | --- |
| V | ROUGE |
| G | NOIR |
| IO26 | JAUNE |
...
**Passez le fil du Buzzer par le Trou 17, puis connectez-le à la carte ESP32.**
...
**Passez le fil de la LED par le Trou 7, puis connectez-le à la carte ESP32.**
...
**Passez le fil du capteur de niveau d'eau par le Trou 13, puis connectez-le à la carte ESP32.**

| Composant | Fil | Broche de la carte ESP32 |
| --- | --- | --- |
| Capteur de niveau d'eau | 3PIN 25cm | io33 |
...
**Passez le fil du capteur d'humidité du sol par le Trou 11, puis connectez-le à la carte ESP32.**

| Composant | Fil | Broche de la carte ESP32 |
| --- | --- | --- |
| Capteur d'humidité du sol | 3PIN 20cm | io32 |
...
Le fil rouge de la pompe à eau est connecté à la borne centrale du module relais, et le fil noir est connecté au GND de la carte ESP32.

De plus, vous devez utiliser un fil Dupont pour connecter la borne gauche du module relais au 3.3V de l'ESP32.
...
Insérez la borne mâle du fil Dupont dans la borne femelle du module relais et serrez-la avec un tournevis.

![image-20250417093737686](../media/image-20250417093737686.png)

Après les étapes ci-dessus, notez que tout le câblage est terminé. Et les fils de la LED, du capteur de niveau d'eau, du capteur d'humidité du sol, du buzzer et de la pompe à eau du relais sont respectivement passés par les trous 7, 11, 13, 17 et 40 gravés sur la planche de tilleul, en préparation de l'installation ultérieure.
...
### Étape 16 Installer le module Buzzer et Led
